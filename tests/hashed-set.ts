...
for (let run = 0; run < 10; ++run) {
  const left = new HashedSet(Point2.fastPack, makePoints());
  const right = new HashedSet(Point2.fastPack, makePoints());

  const disjointMainStart = Date.now();
  const disjointMain = left.isDisjointFrom(right);
  const disjointMainTime = Date.now() - disjointMainStart;
  results.disjoint.times.main.push(disjointMainTime);

  const disjointFilterStart = Date.now();
  const disjointFilter = left.isDisjointFromFilter(right);
  const disjointFilterTime = Date.now() - disjointFilterStart;
  ...
  const supersetMainStart = Date.now();
  const supersetMain = left.isSupersetOf(right);
  const supersetMainTime = Date.now() - supersetMainStart;
  results.superset.times.main.push(supersetMainTime);

  const supersetFilterStart = Date.now();
  const supersetFilter = left.isSupersetOfFilter(right);
  const supersetFilterTime = Date.now() - supersetFilterStart;
